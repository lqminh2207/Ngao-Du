            ...
            <div class="row mb-3" style="padding-top:8px;">
                ...
                <div class="col-8">
                    <div class="text-right col-md-7 ml-auto p-r-0">
                            ...
                            <div class="modal-dialog modal-dialog-centered" style="max-width: 800px;">
                              <div class="modal-content">
                                ...
                                <div class="modal-body" style="text-align: left;">
                                    ...
                                    <input type="text" id="itinerary_id" value="{{ $itinerary_id }}" hidden>
                        ...
                        {{-- <a href="{{ route('places.create', [$tour_id, $place_id]) }}" class="btn btn-info btn-create text-white">Create place</a> --}}
                    </div>
                </div>
                {{-- <input type="hidden" id="itinerary_id" value="{{ $itinerary_id }}"> --}}
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            var datatable = $('#myTable').DataTable({
                ...
                ajax:{
                    type:"POST",
                    url:"{{route('places.getData', [$tour_id, $itinerary_id])}}",
                    data: function (d) {
                        d.search = $('#filter_search').val(),
                        d.place = $('#place_id').val(),
                        d.tour_id = $('#tour_id').val()
                    }
                },
                columns:[
                    {data: 'DT_RowIndex', name: 'DT_RowIndex',width:'5%', orderable: false, searchable:false, class: 'align-middle'},
                    {data:'title', name:'title', width:'25%', orderable: false, searchable:false, class:'text-center align-middle'},
                    {data:'content', name:'content', width:'45%', orderable: false, searchable:false, class:'text-center align-middle'},
                    {data:'action', name:'action', width:'25%', orderable: false, searchable:false, class:'text-center align-middle'},
                ],
            });
    ...
    <script>
        ...
        $(document).on('click', '.btn-edit', function(e) {
            ...
            let id = $(this).data('id')
            let url = '{{ route('places.showInfo', [$tour_id, $itinerary_id ,":id"]) }}'
            console.log(url);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#edit_id').val(response.place.id)
                    $('#itinerary_id').val(response.place.itinerary_id)
                    $('#title').val(response.place.title)
                    CKEDITOR.instances.editor1.setData(response.place.content)
                }
            });
        })
        ...
        $('#btn-submit').click(function (e) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'JSON',
                url: '{{ route('places.store', [$tour_id, $itinerary_id]) }}',
                data: {
        ...
        $('#btn-submit-edit').click(function (e) {
            ...
            let place_id = $('#edit_id').val();
            let url = '{{ route('places.update', [$tour_id, $itinerary_id, ":id"]) }}';
            url = url.replace(':id', place_id);
            console.log(url);
